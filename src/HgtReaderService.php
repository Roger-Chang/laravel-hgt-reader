...
class HgtReaderService
{
    ...
    private static function getElevationAtPosition($fileName, $row, $column) {
        if (!array_key_exists($fileName, self::$openedFiles)) {
            if (!file_exists(storage_path().self::$htgFilesDestination . DIRECTORY_SEPARATOR . $fileName)) {
                throw new \Exception("File '{$fileName}' not exists.");
            }
            $file = fopen(storage_path().self::$htgFilesDestination . DIRECTORY_SEPARATOR . $fileName, "r");
            if ($file === false) {
    ...
    public function getElevation($lat, $lon, &$fName = null) {
        ...
        $fName  = self::getFileName($lat, $lon);
    ...
    private static function getFileName($lat, $lon) {
        $N      = self::getDeg($lat, 2);
        $E      = self::getDeg($lon, 3);

        return (($lat < 0)?"S":"N").$N.(($lon < 0)?"W":"E").$E.".hgt";
    }
